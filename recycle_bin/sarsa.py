#本文使用SARSA算法，训练智能体在冰湖环境中走位
#Q-table相当于一个动作价值函数表，记录每个状态下每个动作的预估价值
#本文具体训练时，使用TD算法，并且行为策略和目标策略不一致
#行为策略和目标策略一致，均使用max(Q_table[state,:])，即贪婪策略
#最终算法评判标准
#1、模拟游戏，最终达到目标的占比
#2、模拟游戏，达到目标的平均动作数
...
for episode in range(20000):
    ...
    while not done:
        ...
        #左边是Table里面的值，右边是玩一步之后，使用Table预估的值，目标是两者差值最小，可以使用mse\
        # 并使用梯度下降，优化table，让差值最小化
        #根据游戏是否结束，计算负梯度
        #如果游戏未结束：负梯度 = r_t + γmax(q(s_t+1)) - q(s,a) ；如果游戏结束：负梯度 = r_t - q(s,a)
        if np.random.uniform(0,1) < exploration_rate:
            action_next = env.action_space.sample()
        else :
            action_next = np.argmax(Q_table[new_state,:])
            
        neg_grad = reward + discount_rate*Q_table[new_state,action_next]*(1-done)\
    ...
    exploration_rate = 0.01 + \
                    (0.6 - 0.01) * np.exp(-0.0005 * episode)
