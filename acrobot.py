...
for episode in range(num_episodes):
    ...
    while not done:
        ...
        #计算损失
        loss = criterion(q_values[0][action], torch.tensor(target_q_value))
...
#此处选择保存模型权重和结构
torch.save(Q_network,'./checkpoints/acrobot_dqn1.pth')

#加载模型，注意设置weights_only=False，因为默认是True，即只加载权重，此时会和上面代码冲突，报错
Q_network = torch.load('./checkpoints/acrobot_dqn1.pth',weights_only=False)
...
for episode in range( estim_count ):
    ...
    print(f'第{episode+1}次模拟,消耗总步数{take_action_count},结果：{"win" if done and reward==0 else "lose"}')
#关闭环境
env.close()
#打印模拟结果
